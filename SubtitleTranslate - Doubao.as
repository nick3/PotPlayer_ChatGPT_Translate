/*
    Real-time subtitle translation for PotPlayer using doubao-seed-translation API
*/

// Plugin Information Functions
string GetTitle() {
    return "{$CP949=Doubao 번역$}"
         + "{$CP950=Doubao 翻譯$}"
         + "{$CP936=Doubao 翻译$}"
         + "{$CP0=Doubao Translate$}";
}
...
string GetDesc() {
    return "Real-time subtitle translation using doubao-seed-translation.";
}

string GetLoginTitle() {
    return "{$CP949=Doubao 모델 및 API 키 구성$}"
         + "{$CP950=Doubao 模型與 API 金鑰配置$}"
         + "{$CP936=Doubao 模型与 API 密钥配置$}"
         + "{$CP0=Doubao Model + API URL and API Key Configuration$}";
}

string GetLoginDesc() {
    return "{$CP949=모델 이름, API 주소, 선택적 nullkey, 지연(ms) 및 재시도 모드(0-3)를 입력하십시오 (예: doubao-seed-translation|https://api.openai.com/v1/chat/completions|nullkey|500|retry1).$}"
         ...
         + "{$CP950=請輸入模型名稱、API 地址、可選的 nullkey、延遲毫秒與重試模式(0-3)（例如: doubao-seed-translation|https://api.openai.com/v1/chat/completions|nullkey|500|retry1）。$}"
         ...
         + "{$CP936=请输入模型名称、API 地址、可选的 nullkey、延迟毫秒和重试模式(0-3)（例如: doubao-seed-translation|https://api.openai.com/v1/chat/completions|nullkey|500|retry1）。$}"
         ...
         + "{$CP0=Please enter the model name, API URL, optional 'nullkey', optional delay in ms, and retry mode 0-3 (e.g., doubao-seed-translation|https://api.openai.com/v1/chat/completions|nullkey|500|retry1).$}"
...
string pre_selected_model = "doubao-seed-translation"; // will be replaced during installation
...
string BuildDoubaoSystemJson(const string &in sourceLangCode, const string &in targetLangCode) {
    ...
    if (sourceLangCode == "")
        return "{\"target_language\":\"" + escapedTarget + "\"}";
    return "{\"source_language\":\"" + escapedSource + "\",\"target_language\":\"" + escapedTarget + "\"}";
}
...
string NormalizeDoubaoLanguage(const string &in language, bool isTarget) {
    string trimmed = language.Trim();
    if (trimmed == "")
        return isTarget ? "zh" : "";

    string lower = ToLower(trimmed);
    if (lower == "auto" || lower == "auto detect" || lower == "autodetect")
        return isTarget ? "zh" : "";

    if (lower == "zh-cn" || lower == "zh_cn" || lower == "zh-hans" || lower == "zh hans" ||
        lower == "chinese (simplified)" || lower == "chinese simplified" || lower == "简体中文" || lower == "中文（简体）" || lower == "簡體中文")
        return "zh";

    if (lower == "zh-tw" || lower == "zh_tw" || lower == "zh-hant" || lower == "zh hant" ||
        lower == "chinese (traditional)" || lower == "chinese traditional" || lower == "繁體中文" || lower == "中文（繁体）")
        return "zh-Hant";
...
string NormalizeDoubaoSourceLanguage(const string &in language) {
    return NormalizeDoubaoLanguage(language, false);
}

string NormalizeDoubaoTargetLanguage(const string &in language) {
    return NormalizeDoubaoLanguage(language, true);
}
...
string Translate(string Text, string &in SrcLang, string &in DstLang) {
    ...
    string sourceLangCode = NormalizeDoubaoSourceLanguage(SrcLang);
    string targetLangCode = NormalizeDoubaoTargetLanguage(DstLang);

    string systemJson = BuildDoubaoSystemJson(sourceLangCode, targetLangCode);

    string startMarker = "";
    string endMarker = "";
    string userMsg = ComposeDoubaoUserPayload(contextSegments, Text, startMarker, endMarker);

    string escapedSystemMsg = JsonEscape(systemJson);
    string escapedUserMsg = JsonEscape(userMsg);

    string requestData = "{\"model\":\"" + selected_model + "\",";
    requestData += "\"messages\":[{\"role\":\"system\",\"content\":\"" + escapedSystemMsg + "\"},";
    requestData += "{\"role\":\"user\",\"content\":\"" + escapedUserMsg + "\"}],";
    ...
    if (context_cache_mode != "off" && !context_cache_disabled_for_session) {
        ...
        string cacheFailure = "";
        if (responsesUrl != "") {
            translation = TranslateWithResponses(responsesUrl, headers, systemJson, userMsg, delayInt, retryModeInt, cacheFailure);
        } else {
...
void OnInitialize() {
    HostPrintUTF8("Doubao translation plugin loaded.\n");
    RefreshConfiguration();
    context_cache_disabled_for_session = false;
    context_cache_disable_key = "";
    if (api_key != "") {
...
// Plugin Finalization
void OnFinalize() {
    HostPrintUTF8("Doubao translation plugin unloaded.\n");
}
